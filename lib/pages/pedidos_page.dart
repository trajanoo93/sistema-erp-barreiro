...
class PedidosPageState extends State<PedidosPage> {
  ...
  await showGeneralDialog(
    ...
    pageBuilder: (context, anim, secondaryAnim) {
      ...
      final sheetWidth = (w * 0.40).clamp(360.0, 640.0); // 40% (min 360, max 640)
        ...
        child: SizedBox(
          ...
          child: ClipRRect(
            ...
            child: Material(
              color: Colors.white,
              elevation: 16,
              child: PedidoDetailDialog(     // usa seu widget atual
  ...
  Future<void> _fetchPedidosSilently() async {
    ...
    try {
      ...
      for (var pedido in newPedidos) {
        final logLine =
            'Pedido ${pedido.id}: descontoGiftCard=${pedido.descontoGiftCard}, JSON=${jsonEncode(pedido.toJson())}\n';
        logSink.write(logLine);
      }
      logSink.write('Pedidos retornados: ${newPedidos.length}\n');
      ...
      if (newPedidos.isNotEmpty) {
        ...
        final List<String> newIdsCanonical =
            deduped.map<String>((p) => _canonicalId(p.id)).toList();
  ...
  Future<void> _processNewPedidos(List<Pedido> newPedidos) async {
    ...
    for (var pedido in newPedidos) {
      ...
      final dataAgendamento = DateTime.tryParse(pedido.dataAgendamento);
      if (dataAgendamento == null) {
        await PedidoService.writeLog(
            'Pedido ${pedido.id} ignorado: data_agendamento inválida (${pedido.dataAgendamento}).',
            context);
        continue;
      }
      final agendamentoDate =
          DateTime(dataAgendamento.year, dataAgendamento.month, dataAgendamento.day);
      ...
      if (agendamentoDate == today) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
          try {
            final produtosParsed = parseProdutos(pedido.produtos);
            final dialog =
                PedidoDetailDialog(pedido: pedido.toJson(), produtosParsed: produtosParsed);
  ...
  _debounceTimer = Timer(const Duration(milliseconds: 350), () async {
    ...
    if (_searchText.isNotEmpty) {
      ...
      // 🔴 Pula o filtro de data se estiver buscando
    } else {
      // 🔹 Só aplica filtro de data se não estiver pesquisando
      if (_startDate != null && _endDate != null) {
        tempList = tempList.where((pedido) {
          final dataStr = pedido.dataAgendamento;
          if (dataStr.isEmpty) return false;
          DateTime? dt = DateTime.tryParse(dataStr);
          if (dt == null && dataStr.contains('/')) {
  ...
  int _compareAgendamento(Pedido a, Pedido b) {
    ...
    final dateTimeAgendamentoA =
        _parseAgendamentoToDateTime(dataAgendamentoA, horarioAgendamentoA);
    final dateTimeAgendamentoB =
        _parseAgendamentoToDateTime(dataAgendamentoB, horarioAgendamentoB);
  ...
  String _formatoDataAgendamento(String iso) {
    ...
    try {
      final manualDate = DateFormat('MMMM dd, yyyy').parse(iso, true);
      return DateFormat('dd/MM/yyyy').format(manualDate);
    } catch (_) {
      return iso;
    }
  }
  ...
  List<Map<String, String>> parseProdutos(String produtosRaw) {
    ...
    for (String item in items) {
      final cleanItem =
          item.trim().endsWith('*') ? item.substring(0, item.length - 1).trim() : item.trim();
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return PedidoCard(
                          ...
                          onStatusChanged: (newStatus) {
                            PedidoService
                                    .updateStatusPedidoBarreiro(pedido, newStatus, context)
                                .then((success) {
                              if (success && mounted) {
                                setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // Linha 2: Datas + Status chips + switch concluídos
        Wrap(
          spacing: 12,
          runSpacing: 12,
          ...
          children: [
            ...
            // Chips de status (scrollável se necessário)
            SizedBox(
              height: 40,
            ...
            Row(
              ...
              children: [
                const Text('Mostrar concluídos', style: TextStyle(fontWeight: FontWeight.w500)),
                const SizedBox(width: 6),
                Switch(
                  value: showCompleted,
...
class PedidoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final agendamentoDate =
        formatDataAgendamento(pedido.dataAgendamento.isNotEmpty ? pedido.dataAgendamento : pedido.data);
